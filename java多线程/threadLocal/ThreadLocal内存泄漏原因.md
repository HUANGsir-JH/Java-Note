...
-   *答案：* 因为这种清理是**被动**的。如果一个线程在执行完任务后放回线程池，且后续很久没有在这个线程上调用该 `ThreadLocal` 的方法，那么那块 `Value` 内存就会一直堆积。（expungeStaleEntry（探测式清理）会将 Entry 的 value 置为 null，并对数组进行 Rehash 重新整理。这种设计不仅是为了释放空间，也是为了维持哈希表的查找效率。）
