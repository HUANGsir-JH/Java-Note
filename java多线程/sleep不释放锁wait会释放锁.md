...
`sleep()` **是“休眠”，**`wait()` **是“等待”**。 最本质的区别在于：`sleep()` **不释放锁，而** `wait()` **会释放锁**。
...
-   `sleep()` 使线程进入 **TIMED\_WAITING** 状态。
    
-   `wait()` 使线程进入 **WAITING**（或带时间的 TIMED\_WAITING）状态。
...
**面试官点评：** 能回答出“锁释放”是及格，能解释清楚“为什么 wait 在 Object 中”是优秀。
