...
编写 Starter 的本质是：**封装业务功能 + 定义配置属性 + 编写自动配置类 + 注册 SPI 接口**。 在 Spring Boot 3 中，必须使用 `META-INF/spring/org.springframework.boot.autoconfigure.AutoConfiguration.imports` 来注册自动配置类。
...
在 `src/main/resources` 下创建目录结构：`META-INF/spring/`。 在该目录下新建文件：`org.springframework.boot.autoconfigure.AutoConfiguration.imports`。 **内容为自动配置类的全路径：**
...
**面试建议：** 提到 Starter 时，顺便提一下 **“命名规范”**：官方推荐 `spring-boot-starter-xxx`，第三方推荐 `xxx-spring-boot-starter`。这显示了你对社区规范的了解。
